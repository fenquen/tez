...
public class HistoryEventHandler extends CompositeService {
     ...
     *
     * @param dagHistoryEvent History event
     * @throws IOException
     */
    public void handleCriticalEvent(DAGHistoryEvent dagHistoryEvent) throws IOException {
        ...
        if (recoveryEnabled && historyEvent.isRecoveryEvent()) {
            recoveryService.handle(dagHistoryEvent);
        }
        if (historyEvent.isHistoryEvent() && shouldLogEvent(dagHistoryEvent)) {
            ...
            historyLoggingService.handle(dagHistoryEvent);
        }
        ...
        if (LOG_CRITICAL_EVENTS.isInfoEnabled()) {
            ...
            LOG_CRITICAL_EVENTS.info("[HISTORY]"
                    + "[DAG:" + dagIdStr + "]"
                    + "[Event:" + dagHistoryEvent.getHistoryEvent().getEventType().name() + "]"
                    + ": " + dagHistoryEvent.getHistoryEvent().toString());
        } else {
