...
public class RootInputInitializerManager {
    ...
    public void runInputInitializers(List<RootInputLeafOutput<InputDescriptor, InputInitializerDescriptor>> inputList) throws TezException {
        for (RootInputLeafOutput<InputDescriptor, InputInitializerDescriptor> input : inputList) {
    ...
    @VisibleForTesting
    protected InputInitializer createInitializer(RootInputLeafOutput<InputDescriptor, InputInitializerDescriptor> input,
                                                 InputInitializerContext context) throws TezException {
        try {
            return dagUgi.doAs((PrivilegedExceptionAction<InputInitializer>) () -> ReflectionUtils
    ...
    private static class InputInitializerCallable implements Callable<List<Event>> {
        ...
        public List<Event> call() throws Exception {
            return ugi.doAs((PrivilegedExceptionAction<List<Event>>) () -> {
                LOG.info(
                ...
                try {
                    TezUtilsInternal.setHadoopCallerContext(appContext.getHadoopShim(), initializerWrapper.vertexId);
                    return initializerWrapper.getInputInitializer().initialize();
                } finally {
                    appContext.getHadoopShim().clearHadoopCallerContext();
                }
            });
    ...
    @SuppressWarnings("rawtypes")
    @VisibleForTesting
    private static class InputInitializerCallback implements FutureCallback<List<Event>> {
        ...
        public void onSuccess(List<Event> eventList) {
            ...
            eventHandler.handle(new VertexEventRootInputInitialized(vertexID, initializer.getInput().getName(), eventList));
        }
        ...
        public void onFailure(Throwable t) {
            ...
            eventHandler.handle(new VertexEventRootInputFailed(vertexID, initializer.getInput().getName(),
    ...
    public static class InitializerWrapper implements VertexStateUpdateListener, TaskStateUpdateListener {
        ...
        private final InputInitializer inputInitializer;
        ...
        InitializerWrapper(RootInputLeafOutput<InputDescriptor, InputInitializerDescriptor> input,
                           InputInitializer inputInitializer, InputInitializerContext context,
        ...
        public InputInitializer getInputInitializer() {
        ...
        public void onStateUpdated(VertexStateUpdate event) {
            if (isComplete()) {
                ...
                try {
                    inputInitializer.onVertexStateUpdated(event);
                } catch (Exception e) {
        ...
        public void onTaskSucceeded(String vertexName, TezTaskID taskId, int attemptId) {
            ...
            int successfulAttempt = vertexSuccessfulAttemptMap.computeIfAbsent(taskId.getId(), k -> attemptId);

            // Run through all the pending events for this srcVertex to see if any of them need to be dispatched.
            List<TezEvent> tezEventList = pendingEvents.get(vertexName);
            if (tezEventList != null && !tezEventList.isEmpty()) {
                List<InputInitializerEvent> toForwardEvents = new LinkedList<InputInitializerEvent>();
                Iterator<TezEvent> eventIterator = tezEventList.iterator();
        ...
        private void sendEvents(List<InputInitializerEvent> events) {
            if (events != null && !events.isEmpty()) {
                try {
                    inputInitializer.handleInputInitializerEvent(events);
                } catch (Exception e) {
                    appContext.getEventHandler().handle(
                            new VertexEventRootInputFailed(vertexId, input.getName(),
                                    new AMUserCodeException(Source.InputInitializer, e)));
