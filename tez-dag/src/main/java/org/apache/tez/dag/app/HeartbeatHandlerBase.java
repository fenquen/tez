...
public abstract class HeartbeatHandlerBase<T> extends AbstractService {
    ...
    public HeartbeatHandlerBase(AppContext appContext, int expectedConcurrency, String name) {
        ...
        this.runningMap = new ConcurrentHashMap<>(16, 0.75f, expectedConcurrency);
    }
    ...
    public void serviceStart() {
        timeOutCheckerThread = new Thread(new PingChecker());
    ...
    private class PingChecker implements Runnable {
        ...
        public void run() {
            while (!Thread.currentThread().isInterrupted()) {
                Iterator<Map.Entry<T, ReportTime>> iterator = runningMap.entrySet().iterator();
