...
public class MRInputAMSplitGenerator extends InputInitializer {
    ...
    public List<Event> initialize() throws Exception {
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug("Time to parse MRInput payload into prot: " + stopWatch.now(TimeUnit.MILLISECONDS));
        }
        ...
        if (LOG.isDebugEnabled()) {
            ...
            LOG.debug("Time converting ByteString to configuration: " + stopWatch.now(TimeUnit.MILLISECONDS));
        }
        ...
        InputSplitInfoMem inputSplitInfo = MRInputHelpers.generateInputSplitsToMem(jobConf, groupSplits, sortSplits, groupSplits ? numTasks : 0);
        stopWatch.stop();

        if (LOG.isDebugEnabled()) {
            LOG.debug("Time to create splits to mem: " + stopWatch.now(TimeUnit.MILLISECONDS));
        }

        List<Event> eventList = Lists.newArrayListWithCapacity(inputSplitInfo.getNumTasks() + 1);
        ...
        eventList.add(configureVertexEvent);
