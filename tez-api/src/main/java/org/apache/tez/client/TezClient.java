...
public class TezClient {
    ...
    private TezClient(String name, TezConfiguration tezConf) {
        this(name, tezConf, tezConf.getBoolean(TezConfiguration.TEZ_AM_SESSION_MODE, TezConfiguration.TEZ_AM_SESSION_MODE_DEFAULT));
    }
    ...
    @Private
    protected TezClient(String name,
    ...
    protected TezClient(String name,
        ...
        this.maxSubmitDAGRequestSizeThroughIPC =
                tezConf.getInt(CommonConfigurationKeys.IPC_MAXIMUM_DATA_LENGTH, CommonConfigurationKeys.IPC_MAXIMUM_DATA_LENGTH_DEFAULT) -
                        tezConf.getInt(TezConfiguration.TEZ_IPC_PAYLOAD_RESERVED_BYTES, TezConfiguration.TEZ_IPC_PAYLOAD_RESERVED_BYTES_DEFAULT);

        Limits.setConfiguration(tezConf);

        LOG.info("Tez Client Version: " + apiVersionInfo);
    }
    ...
    private void setupJavaOptsChecker() {
        if (amConfig.getTezConfiguration().getBoolean(
                TezConfiguration.TEZ_CLIENT_JAVA_OPTS_CHECKER_ENABLED, TezConfiguration.TEZ_CLIENT_JAVA_OPTS_CHECKER_ENABLED_DEFAULT)) {
            String javaOptsCheckerClassName = amConfig.getTezConfiguration().get(TezConfiguration.TEZ_CLIENT_JAVA_OPTS_CHECKER_CLASS, "");
    ...
    private void startClientHeartbeat() {
        ...
        long pollPeriod = TezCommonUtils.
                getAMClientHeartBeatPollIntervalMillis(amConfig.getTezConfiguration(), amClientKeepAliveTimeoutIntervalMillis, 10);
        ...
        if (!isLocal && amClientKeepAliveTimeoutIntervalMillis > 0) {
            amKeepAliveService = Executors.newSingleThreadScheduledExecutor(
                    new ThreadFactoryBuilder().setDaemon(true).setNameFormat("AMKeepAliveThread #%d").build());
    ...
    public synchronized DAGClient submitDAG(DAG dag) throws TezException, IOException {
        DAGClient dagClient = isSession ? submitDAGSession(dag) : submitDAGApplication(dag);
        if (dagClient != null) {
    ...
    public synchronized TezAppMasterStatus getAppMasterStatus() throws TezException, IOException {
        ...
        if (isSession) {
            ...
            applicationId = lastSubmittedAppId;
        }

        Preconditions.checkState(applicationId != null, "Cannot get status without starting an application");

        try {
            ApplicationReport appReport = frameworkClient.getApplicationReport(applicationId);
            switch (appReport.getYarnApplicationState()) {
                ...
                case RUNNING:
                    try {
                        return frameworkClient.getAMStatus(amConfig.getTezConfiguration(), applicationId, getUgi());
                    } catch (TezException | ServiceException e) {
    ...
    public synchronized boolean waitTillReady(long timeout, TimeUnit unit) throws IOException, TezException, InterruptedException {
        ...
        while (true) {
            TezAppMasterStatus tezAppMasterStatus = getAppMasterStatus();

            if (tezAppMasterStatus.equals(TezAppMasterStatus.SHUTDOWN)) {
                throw new SessionNotRunning("TezSession has already shutdown. "
                        + ((diagnostics != null) ? diagnostics : NO_CLUSTER_DIAGNOSTICS_MSG));
            }

            if (tezAppMasterStatus.equals(TezAppMasterStatus.READY)) {
                return true;
            }
            ...
            long sleepTime = Math.min(SLEEP_FOR_READY, endTime - now);
    ...
    DAGClient submitDAGApplication(ApplicationId appId, DAG dag) throws TezException, IOException {
        ...
        try {
            ...
            ApplicationSubmissionContext appSubmitContext =
                    TezClientUtils.createApplicationSubmissionContext(
            ...
            frameworkClient.submitApplication(appSubmitContext);
    ...
    private synchronized Map<String, LocalResource> getTezJarResources(Credentials credentials) throws IOException {
        if (cachedTezJarResources == null) {
            cachedTezJarResources = new HashMap<>();
            usingTezArchiveDeploy = TezClientUtils.setupTezJarsLocalResources(amConfig.getTezConfiguration(), credentials, cachedTezJarResources);
        }
