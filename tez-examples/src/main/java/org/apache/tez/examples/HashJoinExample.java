...
public class HashJoinExample extends TezExampleBase {
    ...
    public static void main(String[] args) throws Exception {
        int status = ToolRunner.run(new Configuration(), new HashJoinExample(), args);
        System.exit(status);
    }

    @Override
    protected void printUsage() {
        System.err.println("Usage: hashjoin <file1> <file2> <numPartitions> <outPath> [" + broadcastOption + "(default false)]");
    }
    ...
    protected int runJob(String[] args, TezConfiguration tezConf,
        ...
        FileSystem fileSystem = outputPath.getFileSystem(tezConf);
    ...
    private DAG createDag(TezConfiguration tezConf,
        ...
        Vertex hashFileVertex =
                Vertex.create(hashSide, ProcessorDescriptor.create(ForwardingProcessor.class.getName()))
                        .addDataSource(inputFile,
                                MRInput.createConfigBuilder(new Configuration(tezConf), TextInputFormat.class, hashPath.toUri().toString())
        ...
        Vertex streamFileVertex =
                Vertex.create(streamingSide, ProcessorDescriptor.create(ForwardingProcessor.class.getName()))
                        .addDataSource(inputFile,
                                MRInput.createConfigBuilder(new Configuration(tezConf), TextInputFormat.class, streamPath.toUri().toString())
                                        .groupSplits(!isDisableSplitGrouping())
                                        .generateSplitsInAM(!isGenerateSplitInClient())
                                        .build());
         ...
         */
        Vertex joinVertex =
                Vertex.create(joiner, ProcessorDescriptor.create(HashJoinProcessor.class.getName()), numPartitions)
                        .addDataSink(joinOutput,
                                MROutput.createConfigBuilder(new Configuration(tezConf), TextOutputFormat.class, outPath.toUri().toString())
                                        .build());
        ...
        Edge stream2Join = Edge.create(streamFileVertex, joinVertex, streamConf.createDefaultEdgeProperty());

        EdgeProperty hashSideEdgeProperty;
        if (doBroadcast) {
            /**
             ...
             * sides. Thus the join task handling that fragment can join both data set fragment
             */
        ...
        Edge hash2Join = Edge.create(hashFileVertex, joinVertex, hashSideEdgeProperty);

        // Connect everything up by adding them to the DAG
        dag.addVertex(streamFileVertex)
                .addVertex(hashFileVertex)
                .addVertex(joinVertex)
                .addEdge(stream2Join)
                .addEdge(hash2Join);
    ...
    public static class ForwardingProcessor extends SimpleProcessor {
        ...
        public void run() throws Exception {
            ...
            // instance and this processor is used in many different DAGs with different names for inputs and outputs
            KeyValueReader keyValueReader = (KeyValueReader) getInputs().values().iterator().next().getReader();
            KeyValueWriter keyValueWriter = (KeyValueWriter) getOutputs().values().iterator().next().getWriter();

            while (keyValueReader.next()) {
                // The data value itself is the join key. Simply write it out as the key.
                // The output value is null.
                keyValueWriter.write(keyValueReader.getCurrentValue(), NullWritable.get());
            }
        }
    }
    ...
    public static class HashJoinProcessor extends SimpleMRProcessor {
        ...
        public void run() throws Exception {
            ...
            KeyValueWriter keyValueWriter = (KeyValueWriter) getOutputs().get(joinOutput).getWriter();

            // create a hash table for the hash side
            Set<Text> keySet = new HashSet<>();
            KeyValueReader hashKvReader = (KeyValueReader) getInputs().get(hashSide).getReader();
            ...
            KeyValueReader streamKvReader = (KeyValueReader) getInputs().get(streamingSide).getReader();
            while (streamKvReader.next()) {
                Text key = (Text) streamKvReader.getCurrentKey();
                if (keySet.contains(key)) {
                    keyValueWriter.write(key, NullWritable.get());
                }
            }
        }
    }
